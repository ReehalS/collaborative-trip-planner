...
export default function ActivityCard({
  ...
  return (
    <div
      onClick={onClick}
      className={`bg-white rounded-card shadow-card border border-surface-200/50 border-l-4 border-l-primary-300 p-5 transition-all duration-200 ${
        onClick
          ? 'cursor-pointer hover:shadow-card-hover hover:border-l-primary-500'
          : ''
      } animate-fade-in`}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <h3 className="font-semibold text-surface-900 text-lg mb-1.5">
          ...
          <div className="flex items-center gap-1.5 text-sm text-surface-500 mb-3">
            <LuMapPin className="w-4 h-4 flex-shrink-0 text-accent-500" />
            <span>
              {activity.address ||
          ...
          {activity.categories && activity.categories.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {activity.categories.map((cat) => (
                <span
                  key={cat}
                  className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-primary-50 text-primary-800"
                >
                  {cat}
                </span>
              ))}
        ...
        {activity.avgScore > 0 && (
          <div className="flex-shrink-0 flex flex-col items-center">
            <div className="w-11 h-11 rounded-full bg-primary-50 border-2 border-primary-200 flex items-center justify-center">
